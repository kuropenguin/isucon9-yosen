...
func getUserSimpleByID(q sqlx.Queryer, userID int64) (UserSimple, error) {
	...
	// return userSimple, err
	if _, ok := UserSimpleMap[userID]; !ok {
		return UserSimple{}, fmt.Errorf("user not found")
	}
	return *UserSimpleMap[userID], nil
}
...
func postBuy(w http.ResponseWriter, r *http.Request) {
	...
	seller := User{}
	err = tx.Get(&seller, "SELECT `address`, `account_name` FROM `users` WHERE `id` = ? FOR UPDATE", targetItem.SellerID)
	if err == sql.ErrNoRows {
		outputErrorMsg(w, http.StatusNotFound, "seller not found")
		tx.Rollback()
		return
	...
	chanPstr := make(chan *APIPaymentServiceTokenRes, 1)
	defer close(chanPstr)
	go func() {
